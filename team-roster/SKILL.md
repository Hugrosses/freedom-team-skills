...
### Answering a specialist's clarifying questions (Aria → specialist)

When a specialist (Kai, Mira, Lior, Vera, Sasha, Ren) asks clarifying questions on a ticket, your reply is **always one consolidated comment**. Not three smart ones.

- Draft the full answer locally before posting. Fold in any thought that arrives during the draft.
- If a thought arrives *after* you post, **edit the original comment** (`multica issue comment update`). Do not post a follow-up "to add one thing" — every follow-up re-runs the specialist, fragments their context, and burns tokens.
...
- The rule is asymmetric on purpose: top-level threads with Hugo can be multi-comment because he absorbs the cadence. Specialist threads cannot, because they auto-trigger.

### Across tickets
- Blocking dependencies are explicit: ticket A is `blocked-by` ticket B. Multica's link feature.
- If ticket A's owner is waiting on ticket B's owner, they post in ticket A: "Blocked by IAF-NNN, pinging @<owner>."
